...
Особые случаи:
- lt_sumwebmaster (скрытая) — восстанавливается из литерала формулы AJ
  (`=681266,48-AE13` → 681266.48), для этого колонка AJ читается FORMULA-рендером;
- cabinet_spend — Σ(ячейка × коэффициент строки 1) по кабинетной зоне (от AR);
...
def verify_worksheet(ws, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ri0 in range(2, len(grid)):  # строки 3..40
        ...
        # Пустая ячейка-операнд в Sheets-арифметике = 0 (например пустой R в
        # формуле C) — повторяем; деление на 0 у eval_expr по-прежнему None.
