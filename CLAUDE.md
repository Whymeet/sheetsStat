...
- Шапка кабинетов VK — в `AR2:CL2`. По каждому `account_name` из `ads_manager.cabinets` ищем колонку в шапке и пишем spent. Кабинеты без колонки в шапке в таблицу НЕ пишутся (раньше складывались «про запас» в `A37+` и бесконечно копились до лимита листа в 1000 строк) — они видны в JSON-отчёте и в счётчике `unmatched` сводки.
